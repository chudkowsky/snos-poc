...
pub async fn collect_single_block_info(block_number: u64, rpc_client: RpcClient) -> (OsBlockInput, BTreeMap<CompiledClassHash, CasmContractClass>, BTreeMap<CompiledClassHash, ContractClass>, HashSet<ContractAddress>, HashSet<ClassHash>, HashMap<ContractAddress, HashSet<StorageKey>>, Option<BlockId>) {
    ...
    log::debug!("Block IDs configured: current={}, previous={:?}", block_number, previous_block_id.map(|id| format!("{:?}", id)).unwrap_or("None".to_string()));
    ...
    log::info!("Successfully Block with {} transactions fetched", block_with_txs.transactions.len());
    ...
    log::info!("previous block: {:?}, older block: {:?}", previous_block, older_block);
    log::info!("Successfully Previous and older blocks fetched");
    ...
    log::info!("Successfully Found {} accessed addresses and {} accessed classes", accessed_addresses.len(), accessed_classes.len());
    
    log::info!("the addressea are: {:?}", accessed_addresses);
    log::info!("the classes are: {:?}", accessed_classes);
    ...
    log::debug!(" Step 8: Converting transactions to blockifier format...");
    let mut txs = Vec::new();
    ...
    log::info!("Successfully Transaction executor created");
    ...
    log::info!("Successfully All transactions executed successfully");
    ...
    log::debug!("  Step 11: Fetching storage proofs...");
    ...
    log::info!("Successfully Got {} previous storage proofs", previous_storage_proofs.len());
    ...
    for (contract_address, storage_proof) in storage_proofs.clone() {
        ...
        log::info!("the contract address: {:?} and the block-id: {:?}", contract_address, block_id);
        ...
        let class_hash = if contract_address == Felt::ONE {
            log::info!("🔧 Special case: Contract address 0x1 detected, setting class hash to 0x0 without RPC call");
            Felt::ZERO
        } else {
    ...
    log::info!("Successfully Processed {} contract storage commitments", address_to_storage_commitment_info.len());
    ...
    log::debug!("  Step 14: Fetching class proofs for {} class hashes...", class_hashes.len());
    ...
    log::info!("Successfully Got {} class proofs", class_proofs.len());
    ...
    log::info!("Successfully Got {} previous class proofs", previous_class_proofs.len());
    ...
    for (class_hash_felt, generic_class) in compiled_classes {
        ...
        let compiled_class_hash = if let Some(&existing_compiled_hash) = class_hash_to_compiled_class_hash.get(&class_hash) {
            ...
            log::info!("Successfully Found compiled class hash in mapping: {:?} -> {:?}", class_hash, compiled_class_hash);
            compiled_class_hash
        } else {
            // 2. Fallback to RPC call if not in mapping
            rpc_calls_made += 1;
            log::info!("⚠️  Class hash {:?} not found in mapping, making RPC call...", class_hash);
            ...
            match state_reader.get_compiled_class_hash_async(class_hash).await {
                Ok(compiled_hash) => {
                    log::info!("Successfully RPC call succeeded: {:?} -> {:?}", class_hash, compiled_hash);
                    compiled_hash
                }
                Err(e) => {
                    log::info!("❌ RPC call failed for class hash {:?}: {}", class_hash, e);
                    continue; // Skip this class if we can't get compiled class hash
                }
            }
        };
        
        compiled_classes_btree.insert(compiled_class_hash, cairo_lang_class);
    }
    
    log::debug!(" Step 17 stats: {} mapping hits, {} RPC calls made", mapping_hits, rpc_calls_made);
    ...
    for (class_hash_felt, generic_class) in deprecated_compiled_classes {
        ...
        let compiled_class_hash = if let Some(&existing_compiled_hash) = class_hash_to_compiled_class_hash.get(&class_hash) {
            ...
            let compiled_class_hash = CompiledClassHash(existing_compiled_hash.into());
            log::info!("Successfully Found deprecated compiled class hash in mapping: {:?} -> {:?}", class_hash, compiled_class_hash);
            compiled_class_hash
        } else {
            // 2. Fallback to RPC call if not in mapping
            deprecated_rpc_calls_made += 1;
            log::info!("⚠️  Deprecated class hash {:?} not found in mapping, making RPC call...", class_hash);
            let state_reader = AsyncRpcStateReader::new(rpc_client.clone(), block_id);
            match state_reader.get_compiled_class_hash_async(class_hash).await {
                Ok(compiled_hash) => {
                    log::info!("Successfully Deprecated RPC call succeeded: {:?} -> {:?}", class_hash, compiled_hash);
                    compiled_hash
                }
                Err(e) => {
                    log::info!("❌ Deprecated RPC call failed for class hash {:?}: {}", class_hash, e);
                    continue; // Skip this class if we can't get compiled class hash
                }
            }
        };
    ...
    log::debug!(" Deprecated classes stats: {} mapping hits, {} RPC calls made", deprecated_mapping_hits, deprecated_rpc_calls_made);
    log::info!("Successfully Converted {} compiled classes and {} deprecated classes", compiled_classes_btree.len(), deprecated_compiled_classes_btree.len());
