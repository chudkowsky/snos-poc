...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    loop {
        ...
        match check_blocks_exist(&rpc_client, &block_set).await {
            Ok(true) => {
                info!("✅ All blocks in set {:?} exist, proceeding with PIE generation", block_set);
                ...
                match process_block_set(&args, &block_set).await {
                    ...
                    Err(e) => {
                        ...
                        // Write error to file
                        let error_file = format!("{}/error_blocks_{}.txt", 
                                                args.output_dir, block_set[0]);
            ...
            Err(e) => {
                warn!("⚠️ Error checking blocks {:?}: {}, retrying in {} seconds", block_set, e, args.interval);
                sleep(Duration::from_secs(args.interval)).await;
            }
        }
    }
...
async fn check_blocks_exist(rpc_client: &RpcClient, blocks: &[u64; 1]) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
...
async fn process_block_set(args: &Args, blocks: &[u64; 1]) -> Result<String, PieGenerationError> {
    ...
    let output_filename = format!("cairo_pie_blocks_{}_{}.zip", 
                                blocks[0], timestamp);
    ...
    match generate_pie(input).await {
        Ok(result) => {
            info!("✅ PIE generation completed for blocks {:?}", result.blocks_processed);
            Ok(output_filename)
        }
        Err(e) => {
