...
fn invoke_v1_to_blockifier(
    ...
    Ok(TransactionConversionResult {
        starknet_api_tx: starknet_api::executable_transaction::Transaction::Account(starknet_api::executable_transaction::AccountTransaction::Invoke(api_txn_real)),
        blockifier_tx: blockifier::transaction::transaction_execution::Transaction::Account(AccountTransaction::new_for_sequencing(again_once)),
    })
}
...
fn invoke_v3_to_blockifier(
    ...
    let api_tx = starknet_api::transaction::InvokeTransaction::V3(starknet_api::transaction::InvokeTransactionV3 {
        ...
        account_deployment_data: starknet_api::transaction::fields::AccountDeploymentData(tx.account_deployment_data.to_vec()),
    });
...
async fn create_class_info(
    ...
    let (blockifier_contract_class, program_length, abi_length, version) = match starknet_contract_class {
        starknet::core::types::ContractClass::Sierra(sierra) => {
            ...
            (contract_class, flattened_sierra.sierra_program.len(), flattened_sierra.abi.len(), SierraVersion::extract_from_program(&sierra.sierra_program).unwrap())
        }
...
async fn declare_v0_to_blockifier(
    ...
    //     declare,
    // )))
    let api_txn_real = starknet_api::executable_transaction::DeclareTransaction::create(api_tx, class_info, &chain_id).unwrap();
    let again_once = starknet_api::executable_transaction::AccountTransaction::Declare(api_txn_real.clone());

    Ok(TransactionConversionResult {
        starknet_api_tx: starknet_api::executable_transaction::Transaction::Account(starknet_api::executable_transaction::AccountTransaction::Declare(api_txn_real)),
        blockifier_tx: blockifier::transaction::transaction_execution::Transaction::Account(AccountTransaction::new_for_sequencing(again_once)),
    })
}
...
async fn declare_v3_to_blockifier(
    ...
    let api_tx = starknet_api::transaction::DeclareTransaction::V3(starknet_api::transaction::DeclareTransactionV3 {
        ...
        account_deployment_data: starknet_api::transaction::fields::AccountDeploymentData(tx.account_deployment_data.clone()),
    });
    let class_info = create_class_info(tx.class_hash, client, block_number).await?;
    // let declare = blockifier::transaction::transactions::DeclareTransaction { tx: api_tx, tx_hash, class_info };
...
fn l1_handler_to_blockifier(
    ...
    let api_txn_real = starknet_api::executable_transaction::L1HandlerTransaction::create(api_tx, &chain_id, paid_fee_on_l1).unwrap();
...
fn deploy_account_v1_to_blockifier(
    ...
    let api_txn_real = starknet_api::executable_transaction::DeployAccountTransaction::create(api_tx, &chain_id).unwrap();
    let again_once = starknet_api::executable_transaction::AccountTransaction::DeployAccount(api_txn_real.clone());

    Ok(TransactionConversionResult {
        starknet_api_tx: starknet_api::executable_transaction::Transaction::Account(starknet_api::executable_transaction::AccountTransaction::DeployAccount(api_txn_real)),
        blockifier_tx: blockifier::transaction::transaction_execution::Transaction::Account(AccountTransaction::new_for_sequencing(again_once)),
    })
}
...
fn deploy_account_v3_to_blockifier(
    ...
    let api_tx = starknet_api::transaction::DeployAccountTransaction::V3(
        starknet_api::transaction::DeployAccountTransactionV3 {
            ...
            constructor_calldata: starknet_api::transaction::fields::Calldata(Arc::new(tx.constructor_calldata.clone())),
            nonce_data_availability_mode: da_mode_core_to_api(tx.nonce_data_availability_mode),
            fee_data_availability_mode: da_mode_core_to_api(tx.fee_data_availability_mode),
            paymaster_data: starknet_api::transaction::fields::PaymasterData(tx.paymaster_data.clone()),
        },
    ...
    Ok(TransactionConversionResult {
        starknet_api_tx: starknet_api::executable_transaction::Transaction::Account(starknet_api::executable_transaction::AccountTransaction::DeployAccount(api_txn_real.clone())),
        blockifier_tx: blockifier::transaction::transaction_execution::Transaction::Account(AccountTransaction::new_for_sequencing(again_once)),
    })
}
